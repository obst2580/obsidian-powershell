...
멀티라인 입력을 지원하는 도구에서는 `Shift + Enter`로 줄바꿈을 넣을 수 있습니다. 기본값은 bracketed newline paste입니다. Claude Code가 특정 상태에서 이를 제출로 처리하면 플러그인 설정의 **Shift+Enter behavior**를 **Claude backslash newline**으로 바꿉니다. 이 모드는 Claude Code의 줄 끝 `\` + Enter 멀티라인 경로를 사용합니다.
