...
멀티라인 입력을 지원하는 도구에서는 `Shift + Enter`로 줄바꿈을 넣을 수 있습니다. 기본값은 xterm paste newline입니다. Claude Code/Codex가 bracketed paste를 켠 상태이면 붙여넣은 줄바꿈으로 전달됩니다. 도구에 따라 다르면 플러그인 설정의 **Shift+Enter behavior**를 Modified Enter, CSI-u, bracketed newline paste, line feed 중 하나로 바꿉니다.
