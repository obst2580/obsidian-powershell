...
Obst Terminal is an Obsidian Desktop plugin that opens a vault-rooted **Agent Console + raw terminal** in the right sidebar. This branch currently reports version `0.6.16`.
...
You can open multiple AI sessions in the same vault. `Open terminal` keeps the existing single-pane behavior, while `Open new AI session` or the Agent Console `+` button creates a new tab in the same Obst Terminal tab group with its own Claude sessionId and Codex threadId. The session title is editable in the Agent Console header. This is intended for project-management roles such as PM, Writer, Analyst, and Reviewer working beside the same vault documents.
...
For multiple AI collaborators, run `Open new AI session` from the command palette or press the Agent Console `+` button. Each session opens as a tab, gets an editable title, and shows short Claude/Codex session identifiers in the subtitle.
